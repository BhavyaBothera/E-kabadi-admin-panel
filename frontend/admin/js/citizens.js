        ...
        openModal({
            ...
            content: `
                ...
                <div class="action-menu">
                    ...
                    ${
                        (!citizen.verified || citizen.status === "pending" || citizen.applicationStatus === "pending_approval")
                            ? `
                                <button
                                    class="action-menu-item approve-action"
                                    id="actionApproveCitizen">
                                    <span>✓</span>
                                    <div>
                                        <strong>Approve Application</strong>
                                        <small>Verify KYC and record audit log</small>
                                    </div>
                                </button>
                                <button
                                    class="action-menu-item danger-action"
                                    id="actionRejectCitizen">
                                    <span>✕</span>
                                    <div>
                                        <strong>Reject Application</strong>
                                        <small>Decline registration and record audit log</small>
                                    </div>
                                </button>
                              `
                            : ""
            ...
            onOpen: () => {
                ...
                const approveBtn = document.getElementById("actionApproveCitizen");
                if (approveBtn) {
                    approveBtn.addEventListener("click", () => {
                        approveBtn.disabled = true;
                        closeModal();
                        if (typeof approveCitizen === "function") {
                            approveCitizen(id, "Admin verified residential address and KYC");
                        } else if (typeof window.approveCitizen === "function") {
                            window.approveCitizen(id, "Admin verified residential address and KYC");
                        }
                        showToast(`${citizen.name} has been approved and verified!`, "success", "Application Approved");
                        setTimeout(() => {
                            loadCitizens();
                            updateStats();
                            renderTable();
                ...
                const rejectBtn = document.getElementById("actionRejectCitizen");
                if (rejectBtn) {
                    rejectBtn.addEventListener("click", () => {
                        rejectBtn.disabled = true;
                        closeModal();
                        const reason = prompt("Enter reason for rejection:", "Aadhaar / Address verification discrepancy");
                        if (reason) {
                            if (typeof rejectCitizen === "function") {
                                rejectCitizen(id, reason);
                            } else if (typeof window.rejectCitizen === "function") {
                                window.rejectCitizen(id, reason);
                            }
                            showToast(`${citizen.name}'s application was rejected.`, "info", "Application Rejected");
                            setTimeout(() => {
                                loadCitizens();
                                updateStats();
                                renderTable();
