...
(function () {
    ...
    function renderNotifications(){
        ...
        try {
            const apps = JSON.parse(localStorage.getItem("ekabadi_applications_v1") || "[]");
            apps.filter(a => a.status === "pending").forEach(a => {
                notifications.unshift({
                    id: a.id,
                    type: "collector",
                    title: `Pending ${a.role === "collector" ? "Collector" : "Citizen"} Approval`,
                    message: `${a.name} submitted registration for verification.`,
